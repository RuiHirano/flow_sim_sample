...
type HarmoVisPlugin struct {

}

func NewHarmoVisPlugin() *HarmoVisPlugin{
	return &HarmoVisPlugin{}
}

func (vis *HarmoVisPlugin) HandleAgents(agents []*util.Agent){
	log.Printf("handle Agents harmovis plugin")
	vis.sendAgents(agents)
}

func (vis *HarmoVisPlugin) RunServer(){
	log.Printf("run server harmovis plugin")
	vis.run()
}
...
// AddAgents : 
func (vis *HarmoVisPlugin)sendAgents(agents []*util.Agent) {
...
func runServer() *gosocketio.Server {
	...
	d := filepath.Join(currentRoot, "plugins/harmovis", "build")
